...
std::vector<uint8_t> SerialProtocolParser::extract_packet_data(const std::vector<uint8_t>& packet) {
    ...
    return std::vector<uint8_t>(packet.begin() + data_start, packet.begin() + data_end);
}
...
std::vector<uint8_t> SerialProtocolParser::parse_serial_data(const std::vector<uint8_t>& new_data) {
    ...
    while (true) {
        ...
        if (!validate_single_packet(single_packet)) {
            std::cerr << "[SerialParser] 单包校验失败，丢弃异常包" << std::endl;
            continue;
        }
        ...
        uint16_t packet_len = (single_packet[2] << 8) | single_packet[1]; // 包长度

        // 5. 处理响应包（固定长度7字节）
        if (packet_len == RESPONSE_PACKET_LEN) {
            uint8_t result = single_packet[5]; // 0=成功，1=失败
            std::cout << "[SerialParser] 收到响应包：" << (result == 0 ? "接收成功" : "接收失败") << std::endl;
        ...
        // 按包计数存储数据（索引从0开始）
        if (packet_count - 1 < cache.received_packets.size()) {
        ...
        if (cache.is_complete()) {
            ...
            std::cout << "[SerialParser] 多包拼接完成，总长度：" << complete_data.size() << std::endl;
        }
    }
