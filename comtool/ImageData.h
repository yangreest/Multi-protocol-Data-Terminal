...
public:
	...
	bool parseFromBytes(const uint8_t* data, size_t size)
	{
		...
		// 1. 解析文件长度 (4 字节)
		fileLength = readInt32(data, offset);
		offset += 4;
		...
		// 7. 解析温度 (4 字节)
		temperature = readFloat(data, offset);
		offset += 4;
		...
		// 13. 解析系统频率 (4 字节)
		systemFrequency = readFloat(data, offset);
		offset += 4;

		// 14. 解析图谱数量 (2 字节)
		spectrumCount = readInt16(data, offset);
		offset += 2;

		// 15. 解析经度 (8 字节)
		longitude = readDouble(data, offset);
		offset += 8;

		// 16. 解析纬度 (8 字节)
		latitude = readDouble(data, offset);
		offset += 8;

		// 17. 解析海拔 (4 字节)
		altitude = readInt32(data, offset);
		offset += 4;
		...
		if (size < offset + spectrumDataLength + 36)
		{
			return false;
		}
		...
		if (spectrumDataLength > 0)
		{
			spectrumData.clear();
			int spDataOffset = 0;
			for (size_t c = 0; c < spectrumCount; c++)
			{
				std::vector<uint8_t> spectrumBytes;
				uint32_t vecDataLength = readInt32(data, offset + spDataOffset + 1);
	 ...
	 */
	std::wstring getStationNameString() const
	{
	...
	std::wstring getWeatherDescription() const
	{
		switch (weather)
		{
		case 0xFF: return L"未记录";
		case 0x01: return L"晴";
		case 0x02: return L"阴";
		case 0x03: return L"雨";
		case 0x04: return L"雪";
		case 0x05: return L"雾";
		case 0x06: return L"雷雨";
		case 0x07: return L"多云";
		default: return L"未知";
		}
	}
	 ...
	 */
	std::wstring getInstrumentManufacturerString() const
	{
	 ...
	 */
	std::wstring getInstrumentModelString() const
	{
...
private:
	...
	{
		return static_cast<int16_t>(data[offset]) |
			(static_cast<int16_t>(data[offset + 1]) << 8);
	}

	static uint32_t readInt32(const uint8_t* data, size_t offset)
	{
	...
	static uint32_t readCRC32(const uint8_t* data, size_t offset)
	{
	...
	{
		uint32_t intVal = static_cast<uint32_t>(data[offset]) |
	...
	{
		uint64_t intVal = static_cast<uint64_t>(data[offset]) |
	 ...
	 */
	std::wstring decodeUnicodeString(const std::vector<uint8_t>& data) const
	{
		std::string result;
		// 初始化一个数组
		std::vector<uint16_t> unicode_data;
		for (size_t i = 0; i + 1 < data.size(); i += 2)
		{
			...
			wchar_t ch = static_cast<wchar_t>(data[i]) |
				(static_cast<wchar_t>(data[i + 1]) << 8);

			unicode_data.push_back(ch);
		}

		// ==========================================
		// 方法1：直接转 C++ 宽字符串 wstring（推荐！）
		// ==========================================
		std::wstring wstr((const wchar_t*)unicode_data.data(), unicode_data.size());
