...
public:
	...
	// 红外温度点阵的宽度
	uint32_t temperatureMatrixWidth;

	// 13. 温度点阵高度 h (int32, 4 字节) [342:345] - 必备
	// 红外温度点阵的高度
	uint32_t temperatureMatrixHeight;

	// 14. 可见光照片数据长度 L1 (int32, 4 字节) [346:349] - 必备
	// 如无可见光照片，该数据为 0
	uint32_t visibleLightDataLength;

	// 15. 红外照片数据长度 L2 (int32, 4 字节) [350:353] - 必备
	// 如无红外照片，该数据为 0
	uint32_t infraredPhotoDataLength;
	...
	bool parseFromBytes(const uint8_t* data, size_t length)
	{
		...
		for (int i = 0; i < k * temperatureMatrixWidth * temperatureMatrixHeight; i++)
		{
			infraredSpectData.push_back(data[offset + i]);
		}
		offset += k * temperatureMatrixWidth * temperatureMatrixHeight;
	 ...
	 */
	std::wstring getDeviceNameString() const
	{
	 ...
	 */
	std::wstring getMeasurementPointNameString() const
	{
